...
{
#if 0
    /* TODO This really needs to be bound up with the RTC on the TI platform
     * because the LoRaMAC offers an NTP-link time synchronization facility which
     * we MUST take advantage of to enable a plethora of interesting time synchronized
     * activities.  Think about localization of sounds.  In our app we should allow
     * uploading of filters that customers create for their specific application.
     */
    SysTime_t deltaTime;
...
SysTime_t SysTimeGetMcuTime( void )
{
    ...
    /* Returns time in ms */
    uint32_t time_ms = TimerGetCurrentTime();
    calendarTime.Seconds = time_ms / 1000;
    calendarTime.SubSeconds = time_ms % 1000;
